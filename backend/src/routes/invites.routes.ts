...
/**
 * POST /api/invites
 * Créer une nouvelle invitation pour une session
...
router.post(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
      ...
      const { session_id, invitee_email, invitee_username, invitee_id } = req.body;

      // Validation: session_id requis
      if (!session_id) {
        res.status(400).json({ error: "L'ID de la session est requis" });
        return;
      }

      // Au moins un identifiant d'invité requis
      ...
      const inviteResult = await client.query(
        ...
        [session_id, inviterId, inviteeId],
      );
